...
public class BEAM_SEARCH_OSSP {
    ...
    public BEAM_SEARCH_OSSP(int gp[], int n, int m, int d, int jp[][], int jm[][], int st[][], int end[][]) {
        noj = n;
        nom = m;
        D = d;
        ...
        while(level<D) {
            ...
            if(level==0) {
                ...
                for(int i=0;i<D;i++) {
                    ...
                    if(globalV[i]==min1 || globalV[i]==min2) {
                        beamSeq[wid][level] = i;
                        check[wid][i] = true;
                        wid++;
                    }
                ...
                for(int j=0;j<beamW;j++) {
                    ...
                    for(int i=0;i<k;i++) {
                        int job = selJ[i]/noj;
                        globalV[selJ[i]] = Math.min(eachJob[job], eachMac[mac[j]]);
        ...
        int ck[][] = new int[beamW][D];
        boolean ck1[][] = new boolean[beamW][D];
        int pos=0;
        ...
        */
        ...
        for (int j = 0;j<beamW; j++) {
            OSSP ossp = new OSSP(seq1[j], noj, nom, jpT, jmS);
            if(j==0) {
